...
One major barrier to scalability is presented by the costs of data generation and storage[c]. % may reframe this a bit
...
First, can the decentralized storage protocol Solid, with which there have been few implementations of large data storage, % is this accurate?
...
Second, to offer usage advantages over existing systems, can the representation of PGS data using the Resource Description Framework (RDF) as Linked Data be accomplished? 
A further aim within this objective is exploring if storage of PGS data as RDF using Header Dictionary Triples (HDT) format~\cite{spec:hdt} provides similar levels of usability of genomic data while optimizing storage efficiency.
...
Additional investigation into how pre-computed privacy-preserving summaries~\cite{taelman_privacyAgg_2020} of genomic data within patient pod ...
...
% Have not gotten to fixing the Gantt chart up yet (I will transfer the on from the git repo to here)
\noindent
\begin{ganttchart}[
  x unit=11.4pt,
  y unit title=12pt,
